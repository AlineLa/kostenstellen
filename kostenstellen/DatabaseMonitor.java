...
public class DatabaseMonitor extends JFrame implements Runnable{
	...
	protected void showDetails() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				selectedConnection = (ConnectionManagerWrapper) getConnectionList().getSelectedValue();
				getJScrollPane().setViewportView(getDetailPanel(selectedConnection));
			}
		});
	}
	...
	private JPanel getDetailPanel(ConnectionManagerWrapper wrapper){
		if (this.detailPanel == null){
			...
			this.detailPanel.add(this.getDetailToolPanel(),BorderLayout.NORTH);
		}
		this.setInfo(wrapper.manager.getConnection());
		return this.detailPanel;
	}
	private JPanel detailToolPanel = null;
	private JPanel getDetailToolPanel() {
		if (this.detailToolPanel == null) {
			this.detailToolPanel = new JPanel();
			this.detailToolPanel.setLayout(new BoxLayout(this.detailToolPanel, BoxLayout.X_AXIS));
			this.detailToolPanel.add(this.getCloseButton());
			this.detailToolPanel.add(this.getClearButton());
		}
